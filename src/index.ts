...
function deepClone<T>(obj: T, seen = new WeakMap()): T {
  ...
  if (Array.isArray(obj)) {
    const clonedArray = obj.map((item) => deepClone(item, seen)) as unknown as T;
...
export function useTinySlice<State, AR extends ActionReducers<State>>(
  slice: ReturnType<typeof createTinySlice<State, AR>>
) {
